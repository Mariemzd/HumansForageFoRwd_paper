% _vMatchingEnv uses the actual reward history experienced by the agents

%% see whether difference vs range explains variance in pExplore
% adapted from predictExploreByRichness to do larger scale simluations fast
...
parpool(9); % leave me some CPUs, I have 10
...
tic,
% then iterate
cd('/Users/mac/Documents/python/ForagingByRichness/Decision_making_models/MATLAB/simulations/test')
addpath('/Users/mac/Documents/python/ForagingByRichness/Decision_making_models/MATLAB/simulations')
...
% for iter =1  
    ...
    try
            ...
            case 1 % foraging
                ...
                for k = 1:nF
                    ...
                    if length(unique(choices{k})) > 1 % if they chose more than 1 option
                        ...
                        % [ll, transmat{k}, ~,~,emat] = fitOreOitHMM({[choices{k};rewarded{k}]},false); % fit
                        % [explores{k}] = hmmviterbi(choices{k},transmat{k},emat)==1; % decode from the model
        ...
        disp('error!!!!')
        q = lasterr
        keyboard(); % give us the last error if we have a problem
...
% see evalAgents_RLvsForagingByRichness for analyses for gridSearch:
%   what params do best, how each impacts stickiness, that kind of thing

% see compareTimes_RLvsForagingByRichness to look at the time constants of
%  the simulated behavior and see how those compare to the real data