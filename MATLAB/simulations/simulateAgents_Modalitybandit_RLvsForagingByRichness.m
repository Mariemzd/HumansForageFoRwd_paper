...
load touchtrials.mat
else 
name = 'eye'; 
load fitRLtoModality2AB_20rounds_eye251111.mat ; 
load eyetrials.mat
end
%%
if ~exist(name, 'dir')
...
for k = 1:length(goodSubjects)
    V{k} = trials(k).RwdProbs/100; %has to be between 0 and 1 %MZ 2025
...
env.nTrials = length(V{1})+1; % this is the actual number, gives us a better match %the +1 is because some people have variable ntrials, it's a quick and dirty fix %MZ 2025
env.nArms = 2;
            ...
            case 3 
                ...
                for k = 1:nWSLS
                    ...
                    if length(unique(choices{k})) > 1 % if they chose more than 1 option
                        ...
                        % [ll, transmat{k}, ~,~,emat] = fitOreOitHMM_2targetsRish({[choices{k};rewarded{k}]},false); % fit
