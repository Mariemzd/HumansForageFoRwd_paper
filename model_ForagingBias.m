...
if isnan(params(4))
    bias = 0;% CAUTION, I"m NOT IMPLEMENTED
else
    bias = params(4);
end
% this is how I figured out how to code it:
% mkChoice = @(v,bias) 1 ./ (1 + exp((v-thresh).*b + bias));
% figure(); hold on; plot(mkChoice([0:0.1:1],-5)); plot(mkChoice([0:0.1:1],5))
%NO - bias does the same thing that threshold does
...
if (thresh >= 0 && thresh <= 2) && (aW >= 0 && aW <= 1) && (b >= 0 && b < maxBeta) && (bias >= -10 && bias <= 10)
    ...
    % make some utilities
    mkChoice = @(v) 1 ./ (1 + exp((v-thresh).*b + bias));
