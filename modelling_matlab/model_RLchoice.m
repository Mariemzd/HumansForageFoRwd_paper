function [ lik, agreement, v, p_explore ] = model_RLchoice( data, params, maxBeta)
...
if ~isnan(params(4))
    bc = params(4); % choice history noise is independent
else
    bc = b; % otherwise, yoke it to the other noise, +1 param CK model
end
...
if (a >= 0 && a <= 1) && (ac >= 0 && ac <= 1) && (b >= 0 && bc >= 0) && (b < maxBeta && bc < maxBeta)
    ...
    % make some utilities
    mkChoice = @(v,ck) exp(v.*b + ck.*bc) ./ sum(exp(v.*b + ck.*bc));
